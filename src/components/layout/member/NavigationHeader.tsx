          ...
          <div className="relative" ref={menuRef}>
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-xl transition-all duration-300 border ${
                menuOpen ? 'bg-white text-[#005a92] border-white' : 'bg-white/10 text-white border-white/10 hover:bg-white/20'
              }`}
            >
            ...
            {menuOpen && (
              <div className="absolute left-0 mt-3 w-[85vw] sm:w-72 bg-white rounded-[1.5rem] shadow-2xl border border-slate-100 py-3 animate-in fade-in zoom-in-95 duration-200 origin-top-left overflow-y-auto max-h-[80vh]">
                <div className="px-2 space-y-1">
                  ...
                  <MenuLink icon={Plus} label="New Project" active={false} onClick={() => handleNav('/project/create')} />
                  ...
                  <MenuLink icon={History} label="Recent Contacts" active={location.pathname === '/contacts'} onClick={() => handleNav('/contacts')} />
                  
                  <MenuLink icon={Files} label="Documents" active={location.pathname === '/documents'} onClick={() => handleNav('/documents')} />
                  
                  <div className="h-px bg-slate-50 my-2 mx-4" />
                  
                  <MenuLink icon={FolderKanban} label="Recent Projects" active={location.pathname === '/projects'} onClick={() => handleNav('/projects')} />
                  <MenuLink icon={Clock} label="Deadlines" active={location.pathname === '/deadlines'} onClick={() => handleNav('/deadlines')} />
                  <MenuLink icon={CheckCircle2} label="Tasks" active={location.pathname === '/tasks'} onClick={() => handleNav('/tasks')} />
                  <MenuLink icon={Calculator} label="Quick Remedies" active={location.pathname === '/quick-remedies'} onClick={() => handleNav('/quick-remedies')} />
                  <MenuLink icon={BrainCircuit} label="AI Insights" active={false} onClick={() => handleNav('/dashboard')} />
                  <MenuLink icon={Compass} label="Tour Demo" active={location.pathname === '/tour'} onClick={() => handleNav('/tour')} />
                </div>
              </div>
            )}
          </div>
          ...
          {showLogo && (
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate('/home')}>
              <div className="w-9 h-9 rounded-xl bg-white flex items-center justify-center shadow-md">
                <FileText className="w-5 h-5 text-[#005a92]" />
              </div>
              <span className="text-white font-semibold text-lg tracking-tight hidden xs:block">Lien Manager</span>
            </div>
          )}
        </div>
            ...
            >
              <div className="w-8 h-8 rounded-full bg-white text-[#005a92] flex items-center justify-center font-bold shadow-inner overflow-hidden text-sm">
                {profile?.user_details?.image ? (
                  ...
                  profile?.user_details?.name?.charAt(0) || "U"
                )}
              </div>
            ...
            {profileOpen && (
              <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="px-5 py-4 bg-slate-50/50 border-b border-slate-100">
                  <p className="text-sm font-semibold text-slate-800 truncate">{profile?.user_details?.name || 'Account'}</p>
